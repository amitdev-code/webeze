            ...
            <div className="mx-auto w-full max-w-6xl px-4 text-center">
                <div className="ltablet:grid-cols-3 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {choices.map((choice) => (
                        <div
                            key={choice.type}
                            className={`dark:hover:bg-muted-800 hover:shadow-muted-300/30 dark:hover:shadow-muted-800/30 group rounded-2xl p-5 transition-all duration-300 hover:bg-white hover:shadow-xl ${true ? 'dark:bg-muted-800 shadow-muted-300/30 dark:shadow-muted-800/30 bg-white shadow-xl' : ''}`}
                        >
                            <img className="rounded-2xl" src={choice.imgSrc} alt={choice.title} />
                            <div className="my-4">
                                <BaseHeading as='h3' className="text-muted-800 dark:text-muted-100 mb-2 font-medium text-xl">{choice.title}</BaseHeading>
